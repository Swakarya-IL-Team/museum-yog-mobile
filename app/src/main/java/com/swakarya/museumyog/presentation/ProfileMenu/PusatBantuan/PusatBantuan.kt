...
@Composable
fun PusatBantuanHeader(navController: NavHostController) {
    Column {
...
@Composable
fun PusatBantuanScreen(navController: NavHostController) {
    val expandedStates = remember { mutableStateListOf(*Array(dataDropdown.size) { false }) }

    Column(
        modifier = Modifier.verticalScroll(rememberScrollState())
    ) {
        PusatBantuanHeader(navController = navController)
...
fun PusatBantuanPreview() {
    MuseumYogTheme {
        PusatBantuanScreen(navController = rememberNavController())
    }
}
